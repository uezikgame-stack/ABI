...
cap = st.sidebar.number_input("CAPITAL ($)", value=1000) # Капитал теперь в USD

assets, rates = get_data_v5(m_sel)
...
if assets:
    ...
    st.dataframe(df_top[["T", "PRICE"]], use_container_width=True, height=200)
    ...
    # ФОРМУЛА ПРОФИТА (Всегда в USD)
    # Используем f_usd напрямую и капитал в USD
    f_profits_usd = [(p_f_usd - item['P_USD']) * (cap / item['P_USD']) for p_f_usd in st.session_state.f_usd]
    ...
    p_fin_usd = f_profits_usd[-1]
    style = "error-card" if p_fin_usd < 0 else "metric-card"
    c3.markdown(f"<div class='{style}'>ПРОФИТ<br><h3>{p_fin_usd:,.2f} $</h3></div>", unsafe_allow_html=True)
    ...
    with col_t:
        # Убрали 0-6 (hide_index=True)
        table_df = pd.DataFrame({
            "ДЕНЬ": [f"День {i+1}" for i in range(7)],
            "ЦЕНА": [f"{p:,.2f} {sign}" for p in f_prices],
            "ПРОФИТ ($)": [f"{pr:,.2f} $" for pr in f_profits_usd]
        })
        st.dataframe(table_df, hide_index=True, use_container_width=True)

    sig = "ПРОДАВАТЬ" if p_fin_usd < 0 else "ПОКУПАТЬ"
    st.markdown(f"<h2 style='text-align:center; color:{'#ff4b4b' if p_fin_usd < 0 else '#00ffcc'} !important; border: 2px solid;'>СИГНАЛ: {sig}</h2>", unsafe_allow_html=True)
